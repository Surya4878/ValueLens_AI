...
export const Step7ReviewResults: React.FC<Step7Props> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#d9e2ec] p-6 sm:p-8 md:p-10 shadow-xs space-y-8 animate-fadeIn">
      {/* Header */}
      <div className="border-b border-[#e5e9f0] pb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="h-[42px] px-5 text-[14px] font-semibold text-white bg-[#0070f2] hover:bg-[#0057d2] rounded-lg shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="h-[42px] px-5 text-[14px] font-semibold text-[#1d2d3e] bg-white border border-[#d9e2ec] rounded-lg hover:bg-slate-50 shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
          >
          ...
          <button
            type="button"
            onClick={onRestart}
            className="h-[42px] px-4.5 text-[14px] font-semibold text-[#0070f2] bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="p-6 bg-slate-50/70 rounded-xl border border-[#d9e2ec]">
          <span className="text-[12px] font-bold text-[#556b82] uppercase block tracking-wider">Current Platform TCO</span>
          <span className="text-2xl sm:text-3xl font-bold text-[#1d2d3e] font-mono block mt-2">
            ${displayCurrentTco.toLocaleString()}
          </span>
          <span className="text-[12px] text-[#556b82] mt-1 block font-medium">per year ({config.name})</span>
        </div>

        <div className="p-6 bg-slate-50/70 rounded-xl border border-[#d9e2ec]">
          <span className="text-[12px] font-bold text-[#556b82] uppercase block tracking-wider">Target Platform TCO</span>
          <span className="text-2xl sm:text-3xl font-bold text-[#0070f2] font-mono block mt-2">
            ${displayTargetTco.toLocaleString()}
          </span>
          <span className="text-[12px] text-[#556b82] mt-1 block font-medium">per year (SAP BTP)</span>
        </div>

        <div className="p-6 bg-emerald-50/70 rounded-xl border border-emerald-200">
          <span className="text-[12px] font-bold text-emerald-800 uppercase block tracking-wider">Annual Savings</span>
          <span className="text-2xl sm:text-3xl font-bold text-emerald-600 font-mono block mt-2">
            ${displaySavings.toLocaleString()}
          </span>
          <span className="text-[12px] text-emerald-700 font-bold mt-1 block">
            {savingsPct.toFixed(2)}% Reduction
          </span>
        </div>

        <div className="p-6 bg-indigo-50/70 rounded-xl border border-indigo-200">
          <span className="text-[12px] font-bold text-indigo-800 uppercase block tracking-wider">5-Year ROI</span>
          <span className="text-2xl sm:text-3xl font-bold text-indigo-900 font-mono block mt-2">
            {displayRoi.toFixed(2)}%
          </span>
          <span className="text-[12px] text-indigo-600 font-bold mt-1 block">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-50/70 border border-[#d9e2ec] rounded-2xl p-6 sm:p-8 space-y-4">
            ...
            <table className="w-full text-[14px] text-left">
              <thead>
                <tr className="border-b border-[#d9e2ec] text-[#556b82]">
                  <th className="py-3 px-2 font-bold">Year</th>
                  <th className="py-3 px-2 font-bold">Annual Savings</th>
                  <th className="py-3 px-2 text-right font-bold">Cumulative Savings</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#e5e9f0] font-mono">
                <tr>
                  <td className="py-3 px-2 font-bold text-[#1d2d3e]">Year 1</td>
                  <td className="py-3 px-2 text-[#556b82]">${displaySavings.toLocaleString()}</td>
                  <td className="py-3 px-2 text-right font-bold text-[#1d2d3e]">${(displaySavings * 1).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="py-3 px-2 font-bold text-[#1d2d3e]">Year 2</td>
                  <td className="py-3 px-2 text-[#556b82]">${displaySavings.toLocaleString()}</td>
                  <td className="py-3 px-2 text-right font-bold text-[#1d2d3e]">${(displaySavings * 2).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="py-3 px-2 font-bold text-[#1d2d3e]">Year 3</td>
                  <td className="py-3 px-2 text-[#556b82]">${displaySavings.toLocaleString()}</td>
                  <td className="py-3 px-2 text-right font-bold text-[#1d2d3e]">${(displaySavings * 3).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="py-3 px-2 font-bold text-[#1d2d3e]">Year 4</td>
                  <td className="py-3 px-2 text-[#556b82]">${displaySavings.toLocaleString()}</td>
                  <td className="py-3 px-2 text-right font-bold text-[#1d2d3e]">${(displaySavings * 4).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="py-3 px-2 font-bold text-[#1d2d3e]">Year 5</td>
                  <td className="py-3 px-2 text-[#556b82]">${displaySavings.toLocaleString()}</td>
                  <td className="py-3 px-2 text-right font-bold text-[#1d2d3e]">${(displaySavings * 5).toLocaleString()}</td>
                </tr>
                <tr className="bg-emerald-100/60">
                  <td className="py-3.5 px-2.5 font-bold text-emerald-900 rounded-l-lg" colSpan={2}>
                    Net 5-Year Benefit (After ${displayMigrationCost.toLocaleString()} Migration)
                  </td>
                  <td className="py-3.5 px-2.5 text-right font-bold text-emerald-700 rounded-r-lg">
      ...
      <div className="bg-gradient-to-r from-blue-50/90 via-indigo-50/50 to-sky-50/80 rounded-2xl border border-blue-200 p-6 sm:p-8 shadow-xs space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-blue-200/70 pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[12px] px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold border border-emerald-200">
                  Value Add (Free) • $0 Cost
                </span>
              </div>
              <h4 className="text-[18px] font-bold text-[#1d2d3e] font-['72',sans-serif] mt-0.5">
          ...
          <a
            href="/intswitch"
            className="inline-flex items-center space-x-2 text-[14px] font-semibold text-[#0070f2] hover:text-blue-800 transition-colors group"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-5 border border-blue-100 shadow-2xs">
            ...
            <span className="text-[13px] text-[#556b82] mt-1 block">
          ...
          <div className="bg-white rounded-xl p-5 border border-blue-100 shadow-2xs">
            ...
            <span className="text-[12px] text-emerald-700 mt-1 block font-semibold">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 text-[13px] bg-white/95 rounded-xl p-4 border border-blue-100">
          <div className="flex items-center space-x-3">
            <span className="font-semibold text-[#556b82]">Current Run-Rate:</span>
            <span className="font-bold text-[#1d2d3e] font-mono text-[14px]">${displayCurrentTco.toLocaleString()} / yr</span>
            ...
            <span className="font-bold text-[#0070f2] font-mono text-[14px]">${displayTargetTco.toLocaleString()} / yr</span>
          </div>
          <div className="text-[13px] text-[#556b82] font-medium">
        ...
        <div className="space-y-3 pt-1">
          <p className="text-[14px] text-[#556b82] leading-relaxed">{config.intSwitch.scopeDescription}</p>
      ...
      <div className="p-6 sm:p-8 bg-gradient-to-r from-indigo-50/90 via-purple-50/50 to-white rounded-2xl border border-indigo-100 space-y-3">
        <div className="flex items-center space-x-3">
          ...
          <h4 className="text-[14px] font-bold text-indigo-950 uppercase tracking-wider">
            AI Strategic Synthesis
          </h4>
        </div>
        <p className="text-[14px] sm:text-[15px] text-[#1d2d3e] leading-relaxed">
      ...
      {/* Key Opportunities */}
      <div className="p-6 sm:p-8 bg-slate-50/70 rounded-2xl border border-[#d9e2ec] space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[#e5e9f0]">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 h-[44px] px-6 text-[14px] font-semibold text-[#1d2d3e] bg-white border border-[#d9e2ec] rounded-lg hover:bg-slate-50 transition-colors group shadow-xs cursor-pointer"
        >
        ...
        <button
          type="button"
          onClick={handleGoToDashboard}
          className="h-[44px] px-6 text-[14px] font-semibold text-white bg-[#0070f2] hover:bg-[#0057d2] rounded-lg shadow-xs transition-all flex items-center gap-2 cursor-pointer"
        >
