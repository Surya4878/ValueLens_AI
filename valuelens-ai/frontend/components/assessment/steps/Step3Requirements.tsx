...
export const Step3Requirements: React.FC<Step3Props> = ({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-50/90 via-indigo-50/60 to-purple-50/70 rounded-2xl border border-blue-200/90 p-7 sm:p-8 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="flex items-start space-x-5">
          <div className="w-12 h-12 rounded-xl bg-[#0070f2] text-white flex items-center justify-center shrink-0 shadow-xs mt-0.5">
            <Crown className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center space-x-2.5">
              <span className="text-[13px] font-bold uppercase tracking-wider text-blue-700">
                Recommended SAP BTP Target
              </span>
              <span className="text-[13px] px-3 py-0.5 rounded-full bg-blue-100 text-blue-800 font-semibold border border-blue-200">
                Determined by Requirements
              </span>
            </div>
            <h3 className="text-[20px] sm:text-[22px] font-bold text-[#1d2d3e] mt-1.5 font-['72',sans-serif]">
              {recommendedEdition}
            </h3>
            <p className="text-[14px] sm:text-[15px] text-[#556b82] mt-2 leading-relaxed max-w-2xl">
              {recommendationReason}
            </p>
          </div>
        </div>

        <div className="shrink-0 bg-white/95 rounded-xl px-6 py-4 border border-blue-100 text-right shadow-xs">
          <span className="text-[13px] font-bold text-[#556b82] uppercase tracking-wider block mb-1">
            Annualized Base Rate
          </span>
          <span className="text-[22px] sm:text-[24px] font-bold text-[#1d2d3e] font-mono">
      ...
      <div className="bg-white rounded-2xl border border-[#d9e2ec] p-7 sm:p-8 shadow-xs space-y-6">
        <div className="flex items-center justify-between border-b border-[#e5e9f0] pb-4">
          <div className="flex items-center space-x-3.5">
            ...
            <h3 className="text-[18px] sm:text-[19px] font-bold text-[#1d2d3e] font-['72',sans-serif]">
              Enterprise Integration Capabilities Checklist
            </h3>
          </div>
          <span className="text-[13px] font-semibold px-3.5 py-1 bg-slate-100 text-[#556b82] rounded-full border border-slate-200">
          ...
          {COMMON_REQUIREMENTS_QUESTIONS.map((q) => {
            ...
            return (
              <div
                ...
                className={`p-5 sm:p-6 rounded-xl border transition-all cursor-pointer select-none flex items-start space-x-4 ${
                  isChecked
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[15px] sm:text-[16px] font-semibold text-[#1d2d3e] leading-snug">
                      {q.question}
                    </span>
                    <span
                      className={`text-[13px] font-bold uppercase px-2.5 py-0.5 rounded-full shrink-0 ${
                  ...
                  <p className="text-[13px] sm:text-[14px] text-[#556b82] mt-2 leading-relaxed">
                    {q.tooltip}
                  </p>
                </div>
              </div>
      ...
      <div className="flex items-center justify-between pt-6">
        <button
          type="button"
          onClick={onBack}
          className="h-12 px-7 text-[15px] font-semibold text-[#1d2d3e] bg-white border border-[#d9e2ec] rounded-xl hover:bg-slate-50 transition-colors flex items-center space-x-2 shadow-xs group"
        >
        ...
        <button
          type="button"
          onClick={onContinue}
          className="h-12 px-7 text-[15px] font-semibold text-white bg-[#0070f2] hover:bg-[#0057d2] rounded-xl shadow-xs transition-all flex items-center space-x-2 group"
        >
