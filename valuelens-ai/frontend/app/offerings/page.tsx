...
export default function OfferingsPage() {
  const orderedSlugs = ['sap-pipo', 'mulesoft', 'sap-neo-cpi', 'boomi'];
  const platforms = orderedSlugs.map((slug) => OFFERINGS_DATA[slug]).filter(Boolean);

  // Helper to render bullet icons matching 18px-20px requirement
  const renderBulletIcon = (iconType: string) => {
    switch (iconType) {
      case 'chart':
        return (
          <svg className="w-[18px] h-[18px] text-[#0070f2]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 space-y-8 md:space-y-10">
      ...
      {/* 2x2 Grid of Middleware Options (4 Options in exact order: PI/PO, MuleSoft, Neo CPI, Boomi) */}
        ...
        {platforms.map((p) => (
          ...
          >
            ...
            <div className="flex items-center justify-center sm:justify-start space-x-3.5 bg-gradient-to-br from-slate-50/90 to-blue-50/40 p-4 sm:p-5 rounded-2xl border border-slate-200/80 shrink-0 min-w-[220px]">
              ...
              <div className="w-[84px] h-[84px] bg-white rounded-xl border border-[#d9e2ec] p-2.5 flex flex-col items-center justify-center shadow-2xs group-hover:scale-105 transition-transform">
                <img src={p.logo} alt={p.name} className="h-9 max-w-[64px] object-contain mb-1.5" />
                <span className="text-[12px] font-semibold text-[#1d2d3e] text-center leading-tight">
                  {p.shortTitle}
                </span>
              </div>
              ...
              <div className="w-[84px] h-[84px] bg-white rounded-xl border border-blue-200/90 p-2.5 flex flex-col items-center justify-center shadow-2xs group-hover:scale-105 transition-transform">
                <img src={p.cloudLogo} alt="SAP Integration Suite" className="h-9 max-w-[64px] object-contain mb-1.5" />
                <span className="text-[11px] font-bold text-[#0070f2] text-center leading-tight">
